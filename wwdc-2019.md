...
- reloadData()가 가장 안전하지만 animating은 불가능한 문제를 이제야 말하는군.
- Diffable Data Source -> iOS 13 이상. ~~이런 쓸모없는 것들~~.
- Snapshort 개념은 흥미롭지만, 단순히 animating이 필요할 뿐이라면 reloadSections()로도 어느 정도는.
- Item이 Hashable 이어야 하는 것, 한가지 타입의 Item 밖에 사용하지 못하는 것 모두 단점.
- dataSource.apply()는 background thread에서 호출해도 된다고. ~~아이고 고맙기도 해라~~.
- [Demo Project](https://developer.apple.com/documentation/uikit/views_and_controls/collection_views/using_collection_view_compositional_layouts_and_diffable_data_sources)
